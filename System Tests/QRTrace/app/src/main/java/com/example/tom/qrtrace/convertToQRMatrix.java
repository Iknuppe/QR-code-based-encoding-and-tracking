...
public class convertToQRMatrix
{
  ...
  public static int[][] convertToQRMatrix(List<Square> finder_patterns, Bitmap qr_image)
  {
    ...
    //finder_patterns = identifyFinderPatterns(finder_patterns, qr_image);
    ...
    double delta_x = finder_patterns.get(ORIGIN).getLength()/7;//3
    double delta_y = finder_patterns.get(ORIGIN).getHeight()/7;//3
  ...
  /* ------------------------------------------------------------------------ */
  public static List<Square> identifyFinderPatterns(List<Square> finder_patterns, Bitmap qr_image)
  {
    ...
    //debugInformation(sorted_finder_patterns);
  ...
  public static List<Square> findSquares(Bitmap qr_image)
  {
    ...
    {
      HORIZONTAL_SCALE_FACTOR = image_dimension_length/20;//10
      VERTICAL_SCALE_FACTOR = image_dimension_length/20;//10
    }
    else
    {
      HORIZONTAL_SCALE_FACTOR = image_dimension_height/30;//13
      VERTICAL_SCALE_FACTOR =image_dimension_height/30;//13
    }

    //System.out.println("Finding squares in "+image_dimension_length+":"+image_dimension_height+"... ("+HORIZONTAL_SCALE_FACTOR+","+VERTICAL_SCALE_FACTOR+")");
            ...
            vertical_counter += VERTICAL_SCALE_FACTOR)
    {
      ...
      {
        //System.out.print(horizontal_counter+"/"+vertical_counter+" ");
...
/*
  ...
  /* ------------------------------------------------------------------------ */
  //private static final int CHECK_TOLERANCE = 8;//40//20
  ...
  private static Coordinate findEdgeLOU(int x_pos, int y_pos, Bitmap qr_image)
  {
    ...
    boolean color_flag = true; //start on black point
